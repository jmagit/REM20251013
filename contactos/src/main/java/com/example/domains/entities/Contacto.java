...
public class Contacto extends AbstractEntity<Contacto> {
	@Id
	private String id;
	...
	private List<@Pattern(regexp = "^(\\d{3}\\s){2}\\d{3}$", message = "debe coincidir con '999 999 999'") String> telefono;
	@Email
	...
	public Contacto(String id, String tratamiento, @NotBlank @Size(min = 2, max = 2) String nombre,
			@Size(min = 2, max = 2) String apellidos, @Pattern(regexp = "^(\\d{3}\\s){2}\\d{3}$") List<String> telefono,
